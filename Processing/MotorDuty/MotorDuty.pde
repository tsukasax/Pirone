...
// クラスのインスタンス
Serial myPort;
graphMonitor MotorpwmGraph;

// シリアル通信用変数定義
float []serial_data = new float [4];
...
float maxRange = 2100;
float offsetY = 0;
String text_title = "Pirone モーターPWM信号（motor_duty）";
...
void setup() {
  ...
  MotorpwmGraph = new graphMonitor(100, 50, 1000, 600);
}


void draw() {
  background(#015F0D);
  MotorpwmGraph.graphDraw(serial_data[0], serial_data[1], serial_data[2], serial_data[3]);
...
class graphMonitor {
    ...
    void graphDraw(float _y1, float _y2, float _y3, float _y4) {
      ...
      fill(255);
      text(nf(serial_data[0], 0, 0), 1145, val_text_posi + 20);
      text(nf(serial_data[1], 0, 0), 1145, val_text_posi + 70);
      text(nf(serial_data[2], 0, 0), 1145, val_text_posi + 120);
      text(nf(serial_data[3], 0, 0), 1145, val_text_posi + 170);
      textFont(myFont_4);
      textSize(18);
      text("motor1",1045,val_text_posi + 10);
      text("motor2",1045,val_text_posi + 60);
      text("motor3",1045,val_text_posi + 110);
      text("motor4",1045,val_text_posi + 160);
      scale(1, -1);
