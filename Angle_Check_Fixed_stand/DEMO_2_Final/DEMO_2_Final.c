...
uint16_t servo3_duty = 1450;            // YAW 初期値0°のDuty値
...
int main() {
    ...
    while (true) {
        ...
        if (roll_conv_data > before_roll_volume + 10 || roll_conv_data < before_roll_volume - 10) {
        ...
        if (pitch_conv_data > servo2_duty + 50 || pitch_conv_data < servo2_duty - 50) {
        ...
        if (yaw_conv_data > servo3_duty + 50 || yaw_conv_data < servo3_duty - 50) {
